...
use std::f64;
use network::{ClientHandle, SocketSender, ClientId};
use utils::Counter;
use game_map::{GameMap, MapIndex, IndexType};
use entities::{Entity, EntityKind, EntityId, PlayerClass, Player};
use rand;
use rand::Rng;
use rand::distributions::{IndependentSample, Range};
use incremental_value::IncrementalValue;
use utils::{time_milliseconds, random_sample};
...
impl Game {
            ...
            if let Some(client) = self.clients.remove(&client_id) {
                ...
                if let Some(player_id) = client.player_id {
                    match self.map.remove_entity_with_id(&player_id) {
                        Ok(_) => { },
                        Err(err) => println!("Attempted to remove player for connection with error: {:?}", err)
                    }
                }
            } else {
    ...
    fn move_player(&mut self, player_id: &EntityId, target: &MapIndex) -> Result<(), GameError> {
        ...
        let squish_action = if let Some((id_at_target, entity_at_target)) = self.map.entity_at(target) {
            // Make sure it's not this player
            match entity_at_target.kind() {
                &EntityKind::Gap { .. } =>
                    SquishAction::KillPlayer { id: player_id.clone() },
                &EntityKind::Player(_) =>
                    SquishAction::KillPlayer { id: id_at_target.clone() },
                &EntityKind::PointOrb { ref points } =>
                    SquishAction::AbsorbOrb { id: id_at_target.clone(), points: points.clone_value() }
            }
        } else { SquishAction::None };
        ...
        match squish_action {
            ...
            SquishAction::AbsorbOrb { id, points } => {
                ...
                if let Some(&mut EntityKind::Player(ref mut player)) = self.map.entity_kind_with_id_mut(player_id) {
        ...
        let gap = Entity::new(
            old_index,
            EntityKind::Gap { spawner: player_id.clone(), spawn_time: time_milliseconds() }
        );
    ...
    fn spawn_points(&mut self, point_count: u64, center: &MapIndex) -> Result<(), GameError> {
        ...
        for _ in 0..(point_count / POINT_CLUMP) {
            ...
            match self.map.entity_kind_at_mut(&tmp_index) {
                ...
                let orb = Entity::new(
                    tmp_index.clone(),
                    EntityKind::PointOrb { points: IncrementalValue::new(POINT_CLUMP) }
                );
...
impl Game {
    ...
    pub fn find_bot_move_position(&self, base_index: &MapIndex, player: &Player) -> Option<MapIndex> {
        ...
        for i in 0..MOVE_TRIES {
            ...
            let rand_relative_index = rand::thread_rng()
                .choose(&player.player_class().move_positions()).unwrap();
