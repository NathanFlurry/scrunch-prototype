...
impl EntityKind {
    ...
    pub fn serialize(&self, init: bool) -> Value {
        match *self {
            ...
            EntityKind::PointOrb { ref points } => Value::Array(vec![Value::from(points.get().clone())])
        }
    }
...
#[derive(Debug)]
pub struct Entity { // TODO: Add some form of destroy data to know how it was destroyed?
...
impl Entity {
    pub fn serialize(&self, id: &EntityId, init: bool) -> Value { // TODO: Convert to an array structure
        // Create the data
        ...
        // Update values
        if init || self.index.is_changed() { data.push(("index".into(), self.index.get().serialize())); }
        if init || self.kind.needs_update() { data.push(("data".into(), self.kind.serialize(init))); }
...
impl PlayerClass {
    pub fn new(color: u64, move_wait: u64, move_positions: Vec<MapIndex>) -> PlayerClass {
        PlayerClass { color, move_wait, move_positions }
    }
...
impl PlayerClass {
    pub fn serialize(&self) -> Value {
        ...
        let move_positions = self.move_positions.iter().map(|x| x.serialize()).collect::<Vec<Value>>().into();
...
impl Player {
    ...
    pub fn serialize(&self, init: bool) -> Value {
        ...
        // Update values
        if init || self.points.is_changed() { data.push(("points".into(), self.points.clone_value().into())); }
